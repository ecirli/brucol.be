        ...
        $myfile = fopen(DIR."_database/".$code.".txt", "w");
			...
			$ss = '';	
			 foreach (unserialize($data['co_authors']) as $ci => $cs) {
					$coas .= '<a href="'.URL.'get-your-certificate-assist.php?code='.$code.'-'.$ci.'" target="_blank">'.$cs.'</a><br>';
				 $ss = 's';
				}
        ...
        $htmlContent = '
          <html>
            <body>
                ...
                <br>
								<br>
                You are welcome to find the digital version of your assistance certificate below. Please click on your name to download the certificate.<br> 
														
								author'.$ss.'
								
								<a href="'.URL.'get-your-certificate-assist.php?code='.$code.'">'.__ucwords($data['name_surname']).'</a><br>
								'.$coas.'
