  ...
  $new = [];
    ...
    while (false !== ($file = readdir($handle))) {
      if ($file != "." && $file != ".." && $file != 'allcap.txt' && strtolower(substr($file, strrpos($file, '.') + 1)) == 'txt') {
          ...
          if ($data['fee'] == 3) continue;
          if ($data['fee'] == 5) continue;
...
<a href="?logout=yes" style="float: right; margin: 10px 0px;">Logout</a>
      ...
      <a href="javascript:;" onclick="send_ok();" style="float: right; margin: 10px 0px;margin-right: 10px;">Send OK</a>
...
//                                               print_r($snames);
      ...
      unset($hasBlue);
                      foreach($new as $i => $s) {
                        ...
                        echo '<td>'.$inr.'</td>';
                        echo '<td><input type="checkbox" class="sendok" name="sendok[]" value="'.$i.'"></td>';
                        echo '<td style="text-align: left;">'.$s['paper_title'];
...
//                                                   echo date('d.m.Y H:i', $lastTime);
                                                  ?>
                                              <?=date('d.m.Y H:i', filemtime(DIR.'_files/'.$ss))?>
              <br>
...
?>
          ...
          echo '</td>';
                        ...
                        echo '<a href="download_paper.php?code='._encrypt($i).'&type=abstract" target="_blank" >First file</a>';
                         ...
                         if (@$data['files']) : 
                                              ...
                                              <a href="_files/<?php echo urlencode($ss) ?>" target="_blank" <?php 
                                                 if (filemtime(DIR.'_files/'.$ss) > $lastTime) {echo ' style="color: blue;" '; $hasBlue[] = filemtime(DIR.'_files/'.$ss);}
                                                 ?> ><?php
                ...
                if (@$data['files_mod']) : echo '<hr>'; ?>
                          ...
                          <a class="uk-button uk-button-default uk-width-1-1 " target="_blank" 
                             style="margin-bottom: 10px; text-transform: lowercase; <?php 
                                                 if (filemtime(DIR.'_files_modified_from_user/'.$data['files_mod']) > $lastTime) {echo ' color: blue; '; $hasBlue[] = filemtime(DIR.'_files_modified_from_user/'.$data['files_mod']);}
                ...
                if (@$data['files_final_version']) : echo '<hr>'; ?>
                          ...
                          <a class="uk-button uk-button-default uk-width-1-1 " target="_blank" 
                             style="margin-bottom: 10px; text-transform: lowercase; <?php 
                                                 if (filemtime(DIR.'_final_proposal_upload/'.$data['files_final_version']) > $lastTime) {echo ' color: blue; '; $hasBlue[] = filemtime(DIR.'_final_proposal_upload/'.$data['files_final_version']); }
...
?>
          ...
          echo '</td>';
                        echo '<td style="font-size: 1px;">';
                        if (count($hasBlue) > 0) {
                          arsort($hasBlue);
                          $hasBlue = array_values($hasBlue);
                           echo $hasBlue[0];
                          unset($hasBlue);
                        }
                        echo '</td>';
  ...
  function makeSortable(table) {
    ...
    function send_ok() {
      ...
      var codes = $('input.sendok:checked').map(function(){
          return $(this).val();
      }).get();
           ...
           $.ajax({
              url: 'index.php',
              data: {__type: 'send_ok_pagination', codes: codes},
              method: 'POST',
              success: function(a) {
                console.log(a);
                  $('body').append(a);
